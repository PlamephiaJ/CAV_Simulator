 ...
 * @LastEditTime: 2024-08-13 20:41:32
...
{
    if (current_car_index_ == 1)
    {
        if (temp_timer_ != nullptr && temp_flag_ == false)
        {
            return;
        }
        if (aeb_msg->data == "AEB" && temp_flag_ == false)
        {
            temp_timer_ = this->create_wall_timer(
                std::chrono::milliseconds(300),
                std::bind(&PurePursuit::func_timer, this));
            return;
        }
        if (aeb_msg->data == "AEB" && temp_flag_ == true)
        {
            start_AEB_ = true;
